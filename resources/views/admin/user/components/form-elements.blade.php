...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('rol'), 'has-success': fields.rol && fields.rol.valid }">
    <label for="rol" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">Rol</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <select v-model="form.rol" v-validate="'required'" class="form-control" id="rol" name="rol">
            <option disabled value="">Selecciona un rol</option>
            <option value="Administrador">Administrador</option>
            <option value="Usuario">Usuario</option>
            <option value="Invitado">Invitado</option>
        </select>
        <div v-if="errors.has('rol')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('rol') }}</div>
    </div>
</div>

<div class="form-group row align-items-center">
    <label for="estado" class="col-form-label text-md-right col-md-2">Activo</label>
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('color'), 'has-success': fields.color && fields.color.valid }">
    ...
    <div class="form-group">
        <input type="color" v-model="form.color" class="form-control" style="width: 100px; height: 40px;">
    </div>
</div>
